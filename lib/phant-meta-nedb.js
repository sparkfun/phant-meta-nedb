...
app.init = function() {
  this.db = new nedb({ filename: path.join(this.directory, 'streams.db'), autoload: true });
};
...
app.list = function(callback, query, offset, limit, sort) {
  ...
  search.sort(nedb_sort).skip(offset).limit(limit).exec(callback);
...
app.get = function(id, callback) {
  ...
  this.db.findOne({_id: id}, function(err, stream) {
...
app.update = function(id, data, callback) {
  ...
  this.db.update({_id: id}, {$set: data}, {}, function(err) {
